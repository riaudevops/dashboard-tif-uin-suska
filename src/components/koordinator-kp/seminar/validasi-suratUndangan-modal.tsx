...
const ValidasiSuratUndanganModal: FC<ValidasiSuratUndanganModalProps> = ({
  ...
  useEffect(() => {
    if (student && isOpen) {
      ...
      console.log("Dokumen step3:", student?.dokumen?.step3);

      const initialDocs =
        student?.dokumen?.step3?.map((doc) => {
        ...
        <div className="overflow-y-auto flex-1 px-3 bg-gray-50 dark:bg-gray-900">
          <div className="py-4">
            ...
            <div className="space-y-2">
              {student.dokumen?.step3 && student.dokumen.step3.length > 0 ? (
                student.dokumen.step3.map((doc) => {
...
//   // Fetch dokumen surat undangan menggunakan TanStack Query
//   const { data: dokumenData, isLoading: isDokumenLoading } = useQuery<
//     Dokumen[]
//   >({
//     queryKey: ["dokumen-surat-undangan", student?.nim],
//     queryFn: () => APISeminarKP.getDokumenSuratUndangan(student?.nim as string),
//     enabled: !!student?.nim && isOpen,
//   });
...
//   // Inisialisasi state dokumen berdasarkan data dari API
//   useEffect(() => {
//     if (dokumenData && isOpen) {
//       const initialDocs = dokumenData.map((doc) => {
//         let isAccepted = false;
//         let isRejected = false;
//         let rejectionReason = "";
...
//                   Memuat dokumen...
//                 </p>
//               ) : dokumenData && dokumenData.length > 0 ? (
//                 dokumenData.map((doc) => {
//                   const docState = documents.find((d) => d.id === doc.id);
//                   return (
//                     <div
