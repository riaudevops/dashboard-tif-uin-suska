...
import { PhoneCall, Mail, MapPin, FingerprintIcon, CircleArrowOutUpRight } from "lucide-react";
...
import { AnimatedSpan, Terminal, TypingAnimation } from "@/components/magic-ui/terminal";
...
const features = [
  ...
  {
    name: "Seminar",
    description: "⏰ Mengintegrasikan proses pendaftaran seminar kerja praktik.",
    className: "col-span-3 lg:col-span-2",
    background: (
      <Terminal>
...
const LandingPage = () => {
  ...
  const handleKeycloakAuth = () => (auth.isAuthenticated ? void auth.signoutRedirect() : void auth.signinRedirect());
  ...
  return (
    <div className="w-screen h-screen">
      {auth.isLoading && <LoadingComponent className="fixed z-50 w-full h-full bg-black bg-opacity-60" />}
      ...
      <div className="w-full h-full">
        <div id="beranda" className="flex items-center justify-center w-screen h-screen text-center lg:pt-8">
          <GridPattern className={cn("[mask-image:radial-gradient(700px_circle_at_center,white,transparent)]", "h-[47%] top-12 opacity-60 z-10")} />
          <RetroGrid />
          ...
          <div className="relative">
            <motion.div className="absolute hidden left-1 lg:-top-32 lg:block" animate={v1FloatingAnimation.animate}>
              <AnimatedContent distance={50} direction="horizontal" reverse={true} config={{ tension: 80, friction: 20 }} animateOpacity scale={0.5} threshold={0.2} delay={2500}>
            ...
            <motion.div className="absolute right-0 hidden lg:-top-32 lg:block" animate={v2FloatingAnimation.animate}>
              <AnimatedContent distance={50} direction="horizontal" reverse={false} config={{ tension: 80, friction: 20 }} animateOpacity scale={0.5} threshold={0.2} delay={3000}>
                <div className="flex flex-col items-center">
            ...
            <motion.div className="absolute hidden lg:left-24 lg:-bottom-14 lg:block" animate={v2FloatingAnimation.animate}>
              <AnimatedContent distance={50} direction="horizontal" reverse={true} config={{ tension: 80, friction: 20 }} animateOpacity scale={0.5} threshold={0.2} delay={3500}>
                <div className="flex flex-col items-center">
            ...
            <motion.div className="absolute hidden lg:right-[88px] lg:-bottom-14 lg:block" animate={v1FloatingAnimation.animate}>
              <AnimatedContent distance={50} direction="horizontal" reverse={false} config={{ tension: 80, friction: 20 }} animateOpacity scale={0.5} threshold={0.2} delay={4000}>
                <div className="flex flex-col items-center">
                  {theme === "dark" ? <BottomRightDark /> : <BottomRightLigth />}
                  <Award className="mt-2" />
                </div>
              </AnimatedContent>
            </motion.div>

            <HashLink smooth to={"#kontributor"} className="flex -mt-28 md:-mt-16 mb-12 justify-center">
              <div className="cursor-pointer inline-flex items-center gap-x-2 bg-background border border-foreground/30 text-xs text-foreground/80 p-2 px-3 rounded-full transition hover:border-foreground/70">
                ...
                <div className="flex items-center gap-x-1">
                  <span className="border-s border-foreground/60 text-green-600 ps-2"> Lihat Kontributor </span>
                  <span data-v-08ea2e1e="" className="flex-shrink-0 size-4 text-green-600">
                    <svg data-v-08ea2e1e="" className="w-[inherit] h-[inherit]" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <AnimatedContent distance={50} direction="vertical" reverse={false} config={{ tension: 80, friction: 20 }} animateOpacity threshold={0.2} delay={1500}>
              <SplitText
            ...
            {theme === "dark" ? (
              <AnimatedContent distance={50} direction="vertical" reverse={false} config={{ tension: 80, friction: 20 }} animateOpacity threshold={0.2} delay={1750}>
                <div className="flex items-center justify-center transition duration-300 hover:-translate-y-1">
                  <CustomGradientText
                    colors={["#5C3C8A", "#C5597E", "#FF8C3A", "#5C3C8A", "#C5597E"]}
                    animationSpeed={6}
                    showBorder={true}
              ...
              <AnimatedContent distance={50} direction="vertical" reverse={false} config={{ tension: 80, friction: 20 }} animateOpacity threshold={0.2} delay={1750}>
                <div className="flex text-white text-md md:text-2xl tracking-tighter items-center justify-center transition duration-300 hover:-translate-y-1">
                  {!auth.isAuthenticated ? (
                    <div onClick={handleKeycloakAuth} className="flex items-center justify-center px-6 py-2 gap-3 bg-gradient-to-bl hover:bg-gradient-to-tl from-pink-600 to-orange-600 cursor-pointer border border-black rounded-3xl">
                      <FingerprintIcon color="white" />
                      <span>Mulai Sekarang</span>
                    </div>
                  ) : (
                    <NavLink to={dashboardURL} className="flex items-center justify-center px-6 py-2 gap-3 bg-gradient-to-bl hover:bg-gradient-to-tl from-pink-600 to-orange-600 cursor-pointer border border-black rounded-3xl">
        ...
        <div id="fitur-kami" className="flex items-center justify-center w-screen h-auto py-16 text-center">
          <Particles
            particleColors={["#A07CFE", "#FE8FB5", "#FFBE7B", "#40ffaa", "#4079ff"]}
            particleCount={250}
            particleSpread={10}
            speed={0.2}
            ...
            <div className="w-full text-foreground flex flex-col items-center pb-10 bg-background leading-8 sm:leading-10">
              <span className="lg:inline flex flex-col tracking-tighter text-3xl md:text-5xl font-semibold px-6 md:px-16 leading-8 sm:leading-6">
                ...
                <div className="relative -left-1.5 sm:left-0 text-center inline px-1.5">
                  <div className="relative z-10 inline">Fitur Unggulan Kami?</div>
                  <div className="absolute bottom-0 left-0 w-full bg-chart-5/50 mb-0.5 h-1/3"></div>
                </div>
              </span>

              <span className="text-sm tracking-tight text-foreground/70 sm:text-lg pt-2 leading-6 sm:leading-8">Dari kemudahan akses sampai efisiensi administrasi, semua layanan kami integrasi biar kamu nggak perlu pusing lagi.</span>
            </div>

            <BentoGrid>
              {features.map((feature, idx) => (
                <BentoCard href={""} cta={""} Icon={"symbol"} key={idx} {...feature} />
              ))}
            </BentoGrid>
          </div>
        </div>
        <div id="kontributor" className="flex px-6 flex-col items-center justify-center w-screen h-screen pt-4">
          <Particles
            particleColors={["#A07CFE", "#FE8FB5", "#FFBE7B", "#40ffaa", "#4079ff"]}
            particleCount={250}
            particleSpread={10}
            ...
            disableRotation={false}
          />
          <GradientText colors={["#5C3C8A", "#C5597E", "#FF8C3A", "#00FF00", "#00CED1", "#FF4500", "#5C3C8A", "#C5597E"]} animationSpeed={2} showBorder={true} className="p-4 text-xl pointer-events-none">
            Kontributor
          </GradientText>
          <div className="w-full z-30 text-center text-foreground flex flex-col items-center pt-7 bg-background leading-8 sm:leading-10">
            <span className="lg:inline flex flex-col tracking-tighter text-3xl md:text-5xl font-semibold px-3 md:px-16 leading-8 sm:leading-6">
              ...
              <div className="relative -left-1.5 sm:left-0 text-center inline px-1.5">
                <div className="relative z-10 inline">dibalik layar web ini?</div>
                <div className="absolute bottom-0 left-0 w-full bg-chart-5/50 mb-0.5 h-1/3"></div>
              </div>
            </span>

            <span className="text-sm tracking-tight text-foreground/70 sm:text-lg pt-2 leading-5 sm:leading-8">Web ini kami bangun sepenuh hati sebagai bentuk kontribusi kami untuk efisiensi layanan administrasi.</span>
          </div>
                    ...
                    <Card>
                      <CardContent className="flex flex-col aspect-square items-center justify-center p-6">
                        <div className="h-8/12 w-full">
                          <img className="rounded-lg h-72 w-full object-cover" alt={contributor.name} src={contributor.image} />
                        </div>
                        <div className="h-4/12 w-full flex flex-col pt-5 gap-4 justify-center items-center">
                          <div className="flex flex-col items-center">
                            <span className="text-base font-semibold tracking-tight">{contributor.name}</span>
                            <span className="text-sm tracking-tight text-foreground/70">{contributor.roles}</span>
                          </div>

                          {/* Social Media Icons */}
                          <div className="flex justify-center gap-4 md:justify-end ">
                            {/* Github Icons */}
                            <a target="_blank" href={contributor.socialProfiles?.github} className="hover:scale-90 active:scale-100">
                            ...
                            <a target="_blank" href={contributor.socialProfiles?.linkedIn} className="hover:scale-90 active:scale-100">
                              <span className="sr-only">LinkedIn</span>
                              <svg className="w-5 h-5 mt-[0.127rem]" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path
                                  fillRule="evenodd"
                            ...
                            <a target="_blank" href={contributor.socialProfiles?.instagram} className="hover:scale-90 active:scale-100">
        ...
        <WarpBackground id="faqs">
          <div className="flex flex-col items-center justify-center w-screen h-screen text-center">
            <div className="w-full z-30 text-center text-foreground flex flex-col items-center bg-background leading-8 sm:leading-10">
              ...
              <span className="text-sm tracking-tight text-foreground/70 sm:text-lg pt-2 px-4 leading-5 sm:leading-8">Berikut adalah beberapa pertanyaan yang sering ditanyakan oleh pengguna.</span>
            </div>
            <Accordion type="single" collapsible className="w-full px-5 space-y-4 lg:w-2/3 mt-8">
              {FaqListItems.map((faq, index) => (
                ...
                >
                  ...
                  <AccordionContent className="text-md text-start text-foreground/70">{faq.answer}</AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
          </div>
        ...
        <footer className="flex flex-col border-t">
            ...
            <div className="grid grid-cols-1 gap-8 text-center md:text-start md:grid-cols-3">
              ...
              <div className="flex flex-col items-center space-y-4 md:items-start">
                <div className="flex items-center gap-1.5 rounded-xl">
                  {theme === "dark" ? <DarkLogoUSR className="w-8 h-8" /> : <LightLogoUSR className="w-8 h-8" />}
                  <span className="text-base font-semibold">
                    dashboard
                    <span className="italic font-medium">.tif-usr</span>
                  </span>
                </div>
                <p className="text-sm">Teknik Informatika. © 2025. All rights reserved.</p>
              </div>
              {/* Pages Section */}
              <div>
          ...
          >
            <Ballpit count={100} gravity={0.7} friction={0.8} wallBounce={0.95} followCursor={true} colors={["#A07CFE", "#FE8FB5", "#FFBE7B", "#40ffaa", "#4079ff"]} />
          </div>
        </footer>
      </div>
    </div>
