...
interface ValidasiPendaftaranSeminarModalProps {
  isOpen: boolean;
  onClose: () => void;
  student: {
    id: number;
...
const ValidasiPendaftaranSeminarModal: FC<
  ValidasiPendaftaranSeminarModalProps
> = ({ isOpen, onClose, student }) => {
  ...
  useEffect(() => {
    if (student && isOpen) {
      ...
      console.log("Dokumen step1:", student?.dokumen?.step1);

      const initialDocs =
        student?.dokumen?.step1?.map((doc) => {
        ...
        <div className="overflow-y-auto flex-1 px-3 bg-gray-50 dark:bg-gray-900">
          <div className="py-4">
            ...
            <div className="space-y-2">
              {student.dokumen?.step1 && student.dokumen.step1.length > 0 ? (
                student.dokumen.step1.map((doc) => {
...
const getNamaDokumen = (jenis: string): string => {
  const names: Record<string, string> = {
    LAPORAN_TAMBAHAN_KP: "Laporan Tambahan Tugas KP",
    FORM_KEHADIRAN_SEMINAR: "Bukti Seminar KP (min. 5 kali)",
    SURAT_KETERANGAN_SELESAI_KP: "Dokumen Surat Keterangan Selesai KP",
  };
  return names[jenis] || "Dokumen Tidak Diketahui";
};

export default ValidasiPendaftaranSeminarModal;
